                ...
                <div class="card shadow m-0 mt-1">
                    ...
                    <figure class="custom-card m-0">
                        <?php if(isset($banner) && $banner != ''){ ?>
                            <img src="<?=htmlspecialchars($banner)?>" class="card-bg">
                        <?php }else{
                            echo '<div style="margin-top: 192px;"></div>';
                        } ?>
                        <figcaption>
                            <img src="<?=isset($meta_image)?htmlspecialchars($meta_image):''?>" alt="<?=isset($card['title'])?htmlspecialchars($card['title']):''?>" class="profile">
                            <h2 class="mb-3"><?=isset($card['title'])?htmlspecialchars($card['title']):''?><span><?=isset($card['sub_title'])?htmlspecialchars($card['sub_title']):''?></span></h2>
                            <p><?=isset($card['description'])?htmlspecialchars($card['description']):''?></p>
                            ...
                            <div class="social-buttons mt-3 text-center"> 
                                ...
                                <a id="download-file" download="<?=isset($card['title'])?htmlspecialchars($card['title']):''?>.vcf" href="#" class="btn btn-sm btn-icon icon-left btn-outline-light col-md-5 mt-1 mr-1 ml-1"><i class="fas fa-download"></i> <?=$this->lang->line('add_to_phone_book')?htmlspecialchars($this->lang->line('add_to_phone_book')):'Add to Phone Book'?></a>
                                <?php } ?>
                                <?php if($card['show_share'] == 1){ ?>
                                <a data-toggle="modal" data-target="#socialShare" href="#" class="btn btn-sm btn-icon icon-left btn-outline-light col-md-5 mt-1 mr-1 ml-1"><i class="fas fa-share-alt"></i> <?=$this->lang->line('share')?htmlspecialchars($this->lang->line('share')):'Share'?></a>
                                <?php } ?>
                            </div>
                        </figcaption>
                    </figure>
